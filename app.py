...
if "user" not in st.session_state:
    ...
    with tab_login:
        with st.form("login_form"):
            active_list = users_df[users_df["status"] == "active"]["name"].tolist() if not users_df.empty else []
            l_name = st.selectbox("מי בשק\"מ?", active_list, index=None, placeholder="בחר שם...")
    ...
    with tab_reg:
        st.info("מלאו פרטים והמנהל יאשר אתכם.")
        with st.form("reg_form", clear_on_submit=True):
            r_name = st.text_input("שם מלא")
            r_pin = st.text_input("קוד אישי (4 ספרות)")
