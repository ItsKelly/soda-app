    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("כניסה"):
            ...
            if not u_match.empty and str(login_pin).strip() == u_match.iloc[0]["pin"]:
                ...
                cookie_manager.set("soda_user_name", u_data['name'], expires_at=datetime.now().replace(year=datetime.now().year + 1))
                st.cache_data.clear()
                st.rerun()
            else:
    ...
    with tabs[0]:
        ...
        with st.expander("💳 טעינת יתרה (הפקדה)"):
            with st.form("pay_f", clear_on_submit=True):
                p_amt = st.number_input("סכום להפקדה (₪)", min_value=1.0, value=10.0, step=1.0)
    ...
    if is_admin:
        with tabs[1]:
            ...
            with col_m1:
                ...
                with st.expander("עדכון מלאי (הוספה/הורדה)"):
                    with st.form("inv_update_form"):
                        st.write("הזן מספר חיובי להוספה, או שלילי להורדה מהמלאי.")
                        q_change = st.number_input("שינוי כמות", value=0, step=1)
                        if st.form_submit_button("עדכן מלאי"):
                            if q_change != 0:
                                new_i = pd.DataFrame([{"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "quantity": q_change}])
            ...
            if not users_df.empty:
                ...
                for _, u in users_df.iterrows():
                    ...
                    d = 0
                    if not ut.empty and "type" in ut.columns:
